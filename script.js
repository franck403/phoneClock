...
let save =  ["7", "10", "13", "2"]
function loadGrid() {
  save = localStorage.getItem('editor').split(',')
}
loadGrid()
console.log(save)
...
function frenchGreeting() {
  ...
  if (hours < 12) {
    return "Bon matin! Nous sommes en " + month + " le " + day + " " + year + ".";
  } else if (hours < 18) {
    return "Bonne après-midi! Nous sommes en " + month + " le " + day + " " + year + ".";
  } else {
...
function batteryUpdate() {
  navigator.getBattery().then((battery) => {
    ...
    batterieChangeValue(".cssBatterie", Math.round(battery.level * 100));
  });
}
// timeout loop 
function seconds() {
